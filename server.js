...
app.get("/api/experiments", async (_req, res) => {
  try {
    ...
    console.error("GET /api/experiments failed:", e);
    // Prisma P2021 => Tabelle fehlt (Migration nicht gelaufen)
    if (e.code === "P2021") return bad(res, "table_not_found", 500);
...
app.get("/api/experiments/:id", async (req, res) => {
  try {
    ...
    console.error("GET /api/experiments/:id failed:", e);
    if (e.code === "P2021") return bad(res, "table_not_found", 500);
    bad(res, "internal_error", 500);
  }
});
...
app.post("/api/experiments", async (req, res) => {
  try {
    const {
      ...
      endTime = null,
      notes = null,
      decision = null,         // Prisma: String? -> nullable erlaubt
      decisionReason = null,   // Prisma: String? -> nullable erlaubt
      arms = []
    } = req.body || {};
    ...
    const armsData = arms.map((a) => {
      ...
      const initialSplit = a.initialSplit == null ? null : Number(a.initialSplit);
      if (
        initialSplit != null &&
    ...
    if (e.code === "P2021") return bad(res, "table_not_found", 500);
    if (e.code === "P2011") return bad(res, "null_constraint_violation", 400);
    bad(res, "internal_error", 500);
  }
});
...
app.patch("/api/experiments/:id", async (req, res) => {
  try {
    ...
    if (e.code === "P2021") return bad(res, "table_not_found", 500);
    console.error("PATCH /api/experiments/:id failed:", e);
    bad(res, "internal_error", 500);
  }
});
...
app.delete("/api/experiments/:id", async (req, res) => {
  try {
    ...
    if (e.code === "P2025") return bad(res, "not_found", 404);
    if (e.code === "P2021") return bad(res, "table_not_found", 500);
    console.error("DELETE /api/experiments/:id failed:", e);
    bad(res, "internal_error", 500);
  }
});
...
app.post("/api/experiments/:id/metrics", async (req, res) => {
  try {
    ...
    if (e.code === "P2021") return bad(res, "table_not_found", 500);
    console.error("POST /api/experiments/:id/metrics failed:", e);
    bad(res, "internal_error", 500);
  }
});
